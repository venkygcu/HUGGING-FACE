...
def _is_torch_tracing() -> bool:
    """Return the TorchScript tracing state without relying on an unexported stub symbol."""
    is_tracing = getattr(torch.jit, "is_tracing", None)
    return bool(is_tracing and is_tracing())
...
def get_vision_cu_seqlens(grid_thw: torch.Tensor, kwargs: dict | None = None) -> torch.Tensor:
    ...
    cu_seqlens = torch.repeat_interleave(grid_thw[:, 1] * grid_thw[:, 2], grid_thw[:, 0]).cumsum(
        dim=0, dtype=grid_thw.dtype if _is_torch_tracing() else torch.int32
    )
...
def get_vision_window_index(
    ...
    if kwargs is not None:
        ...
        if window_index is not None and cu_window_seqlens is not None:
            return cast(torch.Tensor, window_index), cast(torch.Tensor, cu_window_seqlens)
    window_indices: list[torch.Tensor] = []
    cu_window_seqlens_values: list[int] = [0]
    window_index_id = 0
    ...
    for grid_t, grid_h, grid_w in grid_thw.tolist():
        ...
        cu_seqlens_tmp = seqlens.cumsum(0) * spatial_merge_unit + cu_window_seqlens_values[-1]
        cu_window_seqlens_values.extend(cast(list[int], cu_seqlens_tmp.tolist()))
        window_index_id += grid_t * llm_grid_h * llm_grid_w

    window_index = torch.cat(window_indices, dim=0)
    cu_window_seqlens = torch.tensor(cu_window_seqlens_values, device=grid_thw.device, dtype=torch.int32)
